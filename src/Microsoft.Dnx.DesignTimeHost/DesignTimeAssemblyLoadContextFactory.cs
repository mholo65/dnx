...
namespace Microsoft.Dnx.DesignTimeHost
{
    internal class DesignTimeAssemblyLoadContextFactory : IAssemblyLoadContextFactory
    {
        ...
        public IAssemblyLoadContext Create(IServiceProvider serviceProvider)
        {
            ...
            var appHostContext = _compilationEngine.Cache.Get<ApplicationHostContext>(hostContextKey, ctx => 
            {
                ...
                ctx.Monitor(_compilationEngine.NamedCacheDependencyProvider.GetNamedDependency(_project.Name + "_Dependencies"));
